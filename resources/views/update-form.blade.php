...
<body class="min-h-screen flex items-center justify-center bg-gray-100">
    <div class="p-6 mt-10 bg-white shadow-md rounded-lg w-full max-w-lg">
        ...
        <form method="POST" action="{{route ('modify', $reservation->id)}}">
            @csrf
            <div class="flex flex-col justify-between">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700">Habitación reservada:</label>
                    <input type="text" name="selected_room" value="{{$reservation->room->type}}" disabled class="w-full  rounded mt-1">
                </div>
            <div class="mb-4">
                <label class="block text-gray-700">Modifica el tipo de habitación</label>
                <select name="room_type" id="room_type">
                @foreach($reservation->hotel->rooms as $room)
                <option value="{{$room->type}}">{{$room->type}}: {{$room->price}} /noche</option>
                    @endforeach
                </select>
                </div>
